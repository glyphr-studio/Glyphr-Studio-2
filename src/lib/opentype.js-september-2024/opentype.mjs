		...
		for (const prop in cmd) {
			if (['x', 'x1', 'x2'].includes(prop)) {
				...
				this.commands[i][prop] =
					options.y + (flipY ? flipYBase - cmd[prop] : cmd[prop]) * options.scale;
			}
		}
	}
	return this;
	...
	if (coverageTable.format === 1) {
		...
		Table.call(
			...
			[{ name: 'coverageFormat', type: 'USHORT', value: 2 }].concat(
				recordList('rangeRecord', coverageTable.ranges, function (RangeRecord, i) {
					return [
...
function LookupList(lookupListTable, subtableMakers3) {
	Table.call(
		...
		tableList('lookup', lookupListTable, function (lookupTable) {
			...
			check_default.assert(
				!!subtableCallback,
				'Unable to write GSUB lookup type ' + lookupTable.lookupType + ' tables.'
			);
			return new Table(
				'lookupTable',
				[
			...
			for (let lang in translations) {
				...
				if (platformID === 3) {
					...
					if (winLanguage !== void 0) {
						...
						nameRecords.push(
							makeNameRecord(3, 1, winLanguage, nameID, winName.length, winNameOffset)
						);
					}
				}
			}
...
function bgraToRaw(color) {
	return parseInt(
		`0x${toHex(color.b)}${toHex(color.g)}${toHex(color.r)}${toHex(color.a * 255)}`,
		16
	);
}
...
function formatColor(bgra, format = 'hexa') {
	...
	switch (format) {
		...
		case 'hsla':
			return `hsla(${hsl.h.toFixed(2)}, ${hsl.s.toFixed(2)}%, ${hsl.l.toFixed(2)}%, ${parseFloat(
				bgra.a.toFixed(3)
			)})`;
		case 'bgra':
			return bgra;
...
function parseCFFIndex(data, start, conversionFn, version) {
	...
	const objects = [];
	const count =
		version > 1 ? parse_default.getULong(data, start) : parse_default.getCard16(data, start);
...
var FONT_DICT_META = [{ name: 'private', op: 18, type: ['number', 'offset'], value: [0, 0] }];
...
function applyPaintType(font, path) {
	const paintType =
		(font.tables.cff && font.tables.cff.topDict && font.tables.cff.topDict.paintType) || 0;
	if (paintType === 2) {
		path.fill = null;
		path.stroke = 'black';
...
function parseCFFFDSelect(data, start, nGlyphs, fdArrayCount, version) {
	...
	if (format === 0) {
		...
		if (next !== nGlyphs) {
			throw new Error(
				'CFF Table CID Font FDSelect format 3 range has bad final (Sentinal) GID ' + next
			);
		}
	} else {
...
function parseCFFTable(data, start, font, opt) {
	...
	const stringIndex =
		header.formatMajor > 1
			? null
			: parseCFFIndex(data, topDictIndex.endOffset, parse_default.bytesToString);
	const globalSubrIndex = parseCFFIndex(
		data,
		header.formatMajor > 1 ? start + header.size + header.topDictLength : stringIndex.endOffset,
		void 0,
		header.formatMajor
	);
	...
	if (opt.lowMemory) {
		font._push = function (i) {
			...
			font.glyphs.push(
				i,
				glyphset_default.cffGlyphLoader(font, i, parseCFFCharstring, charString, header.formatMajor)
			);
		};
	} else {
...
function makeNameIndex(fontNames) {
	const t = new table_default.Record('Name INDEX', [{ name: 'names', type: 'INDEX', value: [] }]);
	t.names = [];
...
function makeTopDict(attrs, strings, version) {
	const t = new table_default.Record('Top DICT', [{ name: 'dict', type: 'DICT', value: {} }]);
...
function makeCharsets(glyphNames, strings) {
	const t = new table_default.Record('Charsets', [{ name: 'format', type: 'Card8', value: 0 }]);
...
function makePrivateDict(attrs, strings, version) {
	const t = new table_default.Record('Private DICT', [{ name: 'dict', type: 'DICT', value: {} }]);
...
subtableMakers[2] = function makeLookup2(subtable) {
	...
	return new table_default.Table(
		...
		[
			...
			table_default.tableList('seqSet', subtable.sequences, function (sequenceSet) {
				return new table_default.Table(
					'sequenceSetTable',
...
subtableMakers[3] = function makeLookup3(subtable) {
	...
	return new table_default.Table(
		...
		[
			...
			table_default.tableList('altSet', subtable.alternateSets, function (alternateSet) {
				return new table_default.Table(
					'alternateSetTable',
...
subtableMakers[4] = function makeLookup4(subtable) {
	...
	return new table_default.Table(
		...
		[
			...
			table_default.tableList('ligSet', subtable.ligatureSets, function (ligatureSet) {
				return new table_default.Table(
					'ligatureSetTable',
					table_default.tableList('ligature', ligatureSet, function (ligature) {
						return new table_default.Table(
							'ligatureTable',
	...
	if (subtable.substFormat === 1) {
		return new table_default.Table(
			...
			[
				...
				table_default.tableList('sequenceRuleSet', subtable.ruleSets, function (sequenceRuleSet) {
					if (!sequenceRuleSet) {
						return new table_default.Table('NULL', null);
					}
					return new table_default.Table(
						'sequenceRuleSetTable',
						table_default.tableList('sequenceRule', sequenceRuleSet, function (sequenceRule) {
							let tableData = table_default
								.ushortList('seqLookup', [], sequenceRule.lookupRecords.length)
								.concat(
									table_default.ushortList(
										'inputSequence',
										sequenceRule.input,
		...
		return new table_default.Table(
			...
			[
				...
				table_default.tableList('classSeqRuleSet', subtable.classSets, function (classSeqRuleSet) {
					if (!classSeqRuleSet) {
						return new table_default.Table('NULL', null);
					}
					return new table_default.Table(
						'classSeqRuleSetTable',
						table_default.tableList('classSeqRule', classSeqRuleSet, function (classSeqRule) {
							let tableData = table_default
								.ushortList('classes', classSeqRule.classes, classSeqRule.classes.length + 1)
								.concat(
									table_default.ushortList('seqLookupCount', [], classSeqRule.lookupRecords.length)
								);
		...
		let tableData = [{ name: 'substFormat', type: 'USHORT', value: subtable.substFormat }];
	...
	if (subtable.substFormat === 1) {
		let returnTable = new table_default.Table(
			...
			[
				...
				table_default.tableList('chainRuleSet', subtable.chainRuleSets, function (chainRuleSet) {
					return new table_default.Table(
						'chainRuleSetTable',
						table_default.tableList('chainRule', chainRuleSet, function (chainRule) {
							let tableData = table_default
								...
								.concat(
									table_default.ushortList('substitution', [], chainRule.lookupRecords.length)
								);
...
function parseFvarInstance(data, start, axes, names, instanceSize) {
	...
	inst.postScriptName =
		inst.postScriptNameID !== void 0 ? getNameByID(names, postScriptNameID, [6]) : '';
	return inst;
}
...
function parseFvarTable(data, start, names) {
	...
	for (let j = 0; j < instanceCount; j++) {
		instances.push(
			parseFvarInstance(data, instanceStart + j * instanceSize, axes, names, instanceSize)
		);
	}
...
function fontToSfntTable(font) {
	...
	if (!names.unicode.preferredFamily) {
		names.unicode.preferredFamily =
			fontNamesUnicode.fontFamily || fontNamesMacintosh.fontFamily || fontNamesWindows.fontFamily;
	}
	if (!names.macintosh.preferredFamily) {
		names.macintosh.preferredFamily =
			fontNamesMacintosh.fontFamily || fontNamesUnicode.fontFamily || fontNamesWindows.fontFamily;
	}
	if (!names.windows.preferredFamily) {
		names.windows.preferredFamily =
			fontNamesWindows.fontFamily || fontNamesUnicode.fontFamily || fontNamesMacintosh.fontFamily;
	}
	...
	const metaTable =
		font.metas && Object.keys(font.metas).length > 0 ? meta_default.make(font.metas) : void 0;
	const tables = [
		headTable,
		hheaTable,
		maxpTable,
...
function makeSvgImage(text, unitsPerEm) {
	...
	if (viewBoxVal.width > 0 && viewBoxVal.height > 0) {
		if (widthVal.unitType === 1) {
			...
			yScale =
				heightVal.unitType === 1 ? heightVal.valueInSpecifiedUnits / viewBoxVal.height : xScale;
...
function parseGlyfTableAll(data, start, loca, font) {
	...
	for (let i = 0; i < loca.length - 1; i += 1) {
		...
		if (offset !== nextOffset) {
			glyphs.push(
				i,
				glyphset_default.ttfGlyphLoader(font, i, parseGlyph, data, start + offset, buildPath)
			);
		} else {
...
var VariationProcessor = class {
			...
			for (let i = 0; i < this.avar().axisSegmentMaps.length; i++) {
				...
				for (let j = 0; j < segment.axisValueMaps.length; j++) {
					...
					if (j >= 1 && normalized[i] < pair.fromCoordinate) {
						...
						normalized[i] =
							((normalized[i] - prev.fromCoordinate) * (pair.toCoordinate - prev.toCoordinate) +
								Number.EPSILON) /
		...
		for (let h = 0; h < headers.length; h++) {
			...
			for (let a = 0; a < axisCount; a++) {
				...
				if (!header.intermediateStartTuple) {
					...
					factor =
						(factor * normalizedCoords[a] + Number.EPSILON) / (tupleCoords[a] + Number.EPSILON);
				} else {
					if (
						...
						factor =
							(factor * (normalizedCoords[a] - header.intermediateStartTuple[a] + Number.EPSILON)) /
							(tupleCoords[a] - header.intermediateStartTuple[a] + Number.EPSILON);
					} else {
						factor =
							(factor * (header.intermediateEndTuple[a] - normalizedCoords[a] + Number.EPSILON)) /
							(header.intermediateEndTuple[a] - tupleCoords[a] + Number.EPSILON);
					}
				}
			}
			if (factor === 0) {
	...
	getTransform(glyph, coords) {
		...
		if (this.font.tables.hvar) {
			glyph._advanceWidth =
				typeof glyph._advanceWidth !== 'undefined' ? glyph._advanceWidth : glyph.advanceWidth;
			glyph.advanceWidth = transformedGlyph.advanceWidth = Math.round(
				glyph._advanceWidth +
					this.getVariableAdjustment(transformedGlyph.index, 'hvar', 'advanceWidth', coords)
			);
			...
			glyph.leftSideBearing = transformedGlyph.leftSideBearing = Math.round(
				glyph._leftSideBearing +
					this.getVariableAdjustment(transformedGlyph.index, 'hvar', 'lsb', coords)
			);
		}
...
var VariationManager = class {
	...
	getInstanceIndex(coordinates) {
		return this.fvar().instances.findIndex((instance) =>
			Object.keys(coordinates).every((axis) => instance.coordinates[axis] === coordinates[axis])
		);
	}
	/**
	...
	set(instanceIdOrObject) {
		...
		this.font.defaultRenderOptions = Object.assign({}, this.font.defaultRenderOptions, {
...
UnitVector.prototype.distance = function (p1, p2, o1, o2) {
	return (
		this.x * xUnitVector.distance(p1, p2, o1, o2) + this.y * yUnitVector.distance(p1, p2, o1, o2)
	);
};
...
function LOOPCALL(state) {
	...
	for (let i = 0; i < c; i++) {
		exec(state);
		if (false) console.log(++state.step, i + 1 < c ? 'next loopcall' : 'done loopcall', i);
	}
	state.ip = cip;
	state.prog = cprog;
}
...
Tokenizer.prototype.composeRUD = function (RUDs) {
	...
	const hasFAILObject = (obj) =>
		typeof obj === 'object' && Object.prototype.hasOwnProperty.call(obj, 'FAIL');
...
FeatureQuery.prototype.getLookupMethod = function (lookupTable, subtable) {
	...
	switch (substitutionType) {
		...
		case '51':
			return (contextParams) => contextSubstitutionFormat1.apply(this, [contextParams, subtable]);
		case '53':
			return (contextParams) => contextSubstitutionFormat3.apply(this, [contextParams, subtable]);
		default:
			throw new Error(
...
FeatureQuery.prototype.lookupFeature = function (query) {
	...
	if (!feature)
		return new Error(
			`font '${
				(this.font.names.unicode || this.font.names.windows || this.font.names.macintosh).fullName
					.en
...
Bidi.prototype.registerFeatures = function (script, tags) {
	const supportedTags = tags.filter((tag) => this.query.supports({ script, tag }));
