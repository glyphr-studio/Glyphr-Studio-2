...
function makeCard_componentInstanceAttributes(s) {
	...
	content +=
		'<tr><td colspan=2><h3 style="font-size:.9em; color:rgb(153,158,163); margin-top:0px; padding-top:0px;">' +
	...
	content +=
		'<tr><td class="leftcol"> instance name </td>' +
		'<td style="margin-top:0px; padding-top:0px;">' +
		'<input class="namewidth" id="comname" type="text" value="' +
		s.name +
		'" ' +
		'onchange="_UI.multiSelect.paths.name = this.value;">' +
		'</td>' +
		'</tr>';
	...
	content +=
		...
		lockUI('_UI.multiSelect.paths.singleton.xLock', s.xLock, 'xLock') +
		...
		dimSplit() +
		'<div class="lockwrapper">' +
		lockUI('_UI.multiSelect.paths.singleton.yLock', s.yLock, 'yLock') +
	...
	content +=
		...
		'<td>' +
		checkUI('_UI.multiSelect.paths.singleton.flipEW', s.flipEW, true) +
		'</td>' +
		'</tr>';
	...
	content +=
		...
		'<td> flip vertical </td>' +
		'<td>' +
		checkUI('_UI.multiSelect.paths.singleton.flipNS', s.flipNS, true) +
		'</td>' +
		'</tr>';
	...
	rotationhelp +=
		'<br><br>This option changes how this Component Instances is rendered.  Default is re-size, then rotate. When checked, it will be rotate, then re-size. This order has an impact on the Component Instance’s resulting path.';
	rotationhelp +=
	...
	rotationhelp +=
		'<br><br>If a Component Instance is not re-sized, or not rotated, this option has no effect.';
	rotationhelp += '</div>';
...
function updateComponentInstanceDetail(key, value, id) {
	let selci = _UI.multiSelect.paths.singleton;
