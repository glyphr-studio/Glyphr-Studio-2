...
it('Shape: get/set name', () => {
  const s = sampleShape();
  s.name = 'new Name';
  expect(s.name).toBe('new Name');
});

it('Shape: get/set path', () => {
  const s = sampleShape();
...
it('Shape: get/set visible', () => {
  const s = sampleShape();
  s.visible = false;
  expect(s.visible).toBe(false);
});

it('Shape: get/set xLock', () => {
  const s = sampleShape();
  s.xLock = true;
  expect(s.xLock).toBe(true);
});

it('Shape: get/set yLock', () => {
  const s = sampleShape();
  s.yLock = true;
  expect(s.yLock).toBe(true);
});

it('Shape: get/set wLock', () => {
  const s = sampleShape();
  s.wLock = true;
  expect(s.wLock).toBe(true);
});

it('Shape: get/set hLock', () => {
  const s = sampleShape();
  s.hLock = true;
  expect(s.hLock).toBe(true);
});

it('Shape: get/set ratioLock', () => {
  const s = sampleShape();
  s.ratioLock = true;
...
it('Shape: get/set width', () => {
  const s = sampleShape();
  s.width = 100;
  expect(s.width).toBe(100);
});

it('Shape: get/set height', () => {
  const s = sampleShape();
  s.height = 100;
...
it('Shape: makeOpenTypeJSPath', () => {
  const s = sampleShape();
  const re = s.makeOpenTypeJSPath();
  expect(re).toEqual({'commands': [{'type': 'M', 'x': 327, 'y': 500}, {'type': 'C', 'x1': 413, 'y1': 500, 'x2': 484, 'y2': 429, 'x': 484, 'y': 343}, {'type': 'C', 'x1': 484, 'y1': 258, 'x2': 414, 'y2': 186, 'x': 327, 'y': 186}, {'type': 'C', 'x1': 239, 'y1': 186, 'x2': 170, 'y2': 257, 'x': 170, 'y': 343}, {'type': 'C', 'x1': 170, 'y1': 430, 'x2': 240, 'y2': 500, 'x': 327, 'y': 500}, {'type': 'Z'}], 'fill': 'black', 'stroke': null, 'strokeWidth': 1});
});

it('Shape: setShapePosition x', () => {
