...
test('Glyph: reverseWinding', () => {
  const g = sampleGlyph();
  g.reverseWinding();
  expect(g.shapes[0].path.winding).toBe(5);
});

test('Glyph: alignShapes', () => {
  const g = multiTriangleGlyph();
  g.alignShapes('right');
