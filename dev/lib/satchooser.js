...
// -------------------
    ...
    function SatChooser(pv) {
        ...
        // Properties
        pv = pv || {};
        this.clickCallback = pv.clickCallback || function(c) {
 ...
 e.target.style.display = 'none';
        ...
        this.can.onclick = function(e) {
            ...
            let ipx = ((Math.floor(e.offsetX-1) * 4) + (Math.floor(e.offsetY-1) * that.imgData.width * 4));
            let rx = that.imgData.data[ipx+0];
            let gx = that.imgData.data[ipx+1];
            let bx = that.imgData.data[ipx+2];
            let rgbx = 'rgb('+rx+','+gx+','+bx+')';
            let reox = {r: rx, g: gx, b: bx};
    ...
    SatChooser.prototype.getDiscreetWidth = function(w) {
        ...
        let numcells = (255 * 6 / this.step);
        this.cellSize = Math.round((w - (this.borderSize*2)) / numcells);
        let rw = ((this.cellSize * numcells) + (this.borderSize*2) + this.pointerSize);
    ...
    SatChooser.prototype.show = function(pv) {
        ...
        let cs = this.can.style;
        let offset = getOffset(pv.elem);

        function getOffset(obj) {
            let total = obj.offsetParent ? getOffset(obj.offsetParent) : {'offsetTop': 0, 'offsetLeft': 0};
    ...
    SatChooser.prototype.draw = function() {
        ...
        let color = {};
        let step = this.step;
        let numcol = (255 * 6 / step);
        let colw = Math.round((fw - ps - (bs*2)) / numcol);
        ...
        function drawOneColumn() {
            let rstep = (127-color.r) / numcol;
            let gstep = (127-color.g) / numcol;
            let bstep = (127-color.b) / numcol;

            for (let i=0; i<=numcol; i++) {
                let tr = rgbSan(color.r + (i*rstep));
                let tg = rgbSan(color.g + (i*gstep));
                let tb = rgbSan(color.b + (i*bstep));
