...
_UI.multiSelect.shapes.deleteShapes = function() {
  ...
  if (sels.length === 0) _UI.multiSelect.shapes.clear();
    ...
    const singleshape = _UI.multiSelect.shapes.getSingleton();
...
_UI.multiSelect.shapes.isOverBoundingBoxHandle = function(px, py) {
  ...
  const c = isOverBoundingBoxHandle(px, py, this.getGlyph().maxes, _UI.multiSelectThickness);
...
_UI.multiSelect.shapes.draw_RotationAffordance = function() {
  ...
  if (this.members.length === 1) {
    ss = this.members[0];
    const accent = ss.objType === 'ComponentInstance'? _UI.colors.green : _UI.colors.blue;
